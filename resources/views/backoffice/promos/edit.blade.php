...
@php
    $pageTitle = 'Edit Promo - Back Office ATG POS';

    $existingRequirements = $promo->requirements->map(function ($requirement) {
        return [
            'product_variant_id' => (string) $requirement->product_variant_id,
            'qty' => (float) $requirement->qty,
        ];
    })->values();

    $existingRewards = $promo->rewards->map(function ($reward) {
        return [
            'reward_type' => $reward->reward_type,
            'reward_value' => (float) $reward->reward_value,
            'product_variant_id' => $reward->product_variant_id ? (string) $reward->product_variant_id : '',
...
@section('content')
                    ...
                    <div class="form-grid">
                        ...
                        <div class="field">
                            ...
                            <select name="status" id="status" required>
                                <option value="draft" @selected(old('status', $promo->status) === 'draft')>Draft</option>
                                <option value="active" @selected(old('status', $promo->status) === 'active')>Active</option>
                                <option value="discontinued" @selected(old('status', $promo->status) === 'discontinued')>Discontinued</option>
                            </select>
                        </div>
                    </div>
                </div>
                ...
                <div class="section-card">
                    ...
                    <div class="form-grid">
                        <div class="field">
                            ...
                            <input type="date" name="start_date" id="start_date" value="{{ old('start_date', $promo->start_date?->format('Y-m-d')) }}">
                        </div>

                        <div class="field">
                            <label for="end_date">Promo End</label>
                            <input type="date" name="end_date" id="end_date" value="{{ old('end_date', $promo->end_date?->format('Y-m-d')) }}">
                        </div>

                        <div class="field">
                            <label for="start_time">Start Hour</label>
                            <input type="time" name="start_time" id="start_time" value="{{ old('start_time', $promo->start_time ? substr($promo->start_time, 0, 5) : '') }}">
                        </div>

                        <div class="field">
                            <label for="end_time">End Hour</label>
                            <input type="time" name="end_time" id="end_time" value="{{ old('end_time', $promo->end_time ? substr($promo->end_time, 0, 5) : '') }}">
                        </div>
                        ...
                        <div class="field">
                            ...
                            <label class="check-card">
                                <input
                                    ...
                                    value="1"
                                    id="all_day"
                                    @checked(old('all_day', empty($promo->start_time) && empty($promo->end_time)))
                                >
                            ...
                            <div class="days-grid">
                                @foreach($dayOptions as $dayValue => $dayLabel)
                                    <label class="day-check">
                                        <input type="checkbox" name="active_days[]" value="{{ $dayValue }}" @checked(in_array($dayValue, old('active_days', $promo->active_days ?? []), true))>
                                        {{ $dayLabel }}
                                    </label>
                                @endforeach
                            </div>
    ...
    <script>
        ...
        const existingRequirements = @json($existingRequirements);
        const existingRewards = @json($existingRewards);
        ...
        if (existingRequirements.length > 0) {
            existingRequirements.forEach((row) => addRequirement(row));
        } else {
            addRequirement();
        }

        if (existingRewards.length > 0) {
            existingRewards.forEach((row) => addReward(row));
        } else {
            addReward();
        }
    </script>
