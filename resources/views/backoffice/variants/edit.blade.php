...
<head>
    ...
    <title>Edit Variant - Back Office ATG POS</title>
    <style>
        body {
            margin: 0;
...
<body>
    <div class="wrap">
        ...
        <div class="card">
            <form method="POST" action="{{ route('backoffice.variants.update', $variant->id) }}">
                @csrf
                @method('PUT')
                ...
                <div class="field">
                    <label>Code</label>
                    <input type="text" name="code" value="{{ old('code', $variant->code) }}" required>
                </div>

                <div class="grid">
                    <div class="field">
                        <label>Price Dine In</label>
                        <input type="number" name="price_dine_in" min="0" step="0.01" value="{{ old('price_dine_in', $variant->price_dine_in ?? $variant->price) }}" required>
                    </div>

                    <div class="field">
                        <label>Price Delivery</label>
                        <input type="number" name="price_delivery" min="0" step="0.01" value="{{ old('price_delivery', $variant->price_delivery ?? $variant->price) }}" required>
                    </div>
                </div>
                ...
                <div class="field">
                    ...
                    <select name="is_active" required>
                        <option value="1" @selected(old('is_active', (string) $variant->is_active) == '1')>Active</option>
                        <option value="0" @selected(old('is_active', (string) $variant->is_active) == '0')>Inactive</option>
                    </select>
                </div>
