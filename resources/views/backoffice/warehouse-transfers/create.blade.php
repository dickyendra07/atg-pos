...
<head>
    ...
    <title>Transfer Gudang ke Outlet - Back Office ATG POS</title>
    <style>
        body {
            margin: 0;
...
<body>
    <div class="wrap">
        ...
        <div class="card">
            ...
            <form method="POST" action="{{ route('backoffice.warehouse-transfers.store', $warehouse) }}">
                @csrf
                ...
                <div class="field">
                    ...
                    <select name="ingredient_id" required>
                        ...
                        @foreach($stockBalances as $stock)
                            <option value="{{ $stock->ingredient_id }}" @selected(old('ingredient_id') == $stock->ingredient_id)>
                                {{ $stock->ingredient->name ?? '-' }} - {{ $stock->ingredient->category->name ?? '-' }} - {{ $stock->ingredient->unit ?? '-' }} | Stok tersedia: {{ number_format((float) $stock->qty_on_hand, 0, ',', '.') }}
                            </option>
                        @endforeach
                    </select>
                </div>
                ...
                <div class="field">
                    ...
                    <input type="number" name="qty" min="0.01" step="0.01" value="{{ old('qty') }}" required>
                </div>
