                ...
                <div class="field">
                    ...
                    <div style="display:grid;grid-template-columns:repeat(2,1fr);gap:10px;">
                        @foreach($outlets as $outlet)
                            <label style="display:flex;align-items:center;gap:8px;border:1px solid #e5e7eb;padding:12px;border-radius:12px;">
                                <input
                                    type="checkbox"
                                    name="outlet_ids[]"
                                    value="{{ $outlet->id }}"
                                    @checked($product->outlets->contains('id', $outlet->id))
                                >
                                {{ $outlet->name }}
                            </label>
                        @endforeach
